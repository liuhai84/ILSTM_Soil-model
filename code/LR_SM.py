...
def generatedatafluxnet(datadir,dataname):
    ...
    VPD_ERA = pd.DataFrame(z.loc[:, ['VPD_ERA']])
    ...
    ds1 = pd.concat([monthtimedata,daytimedata,Tsdata,Pdata,SWCdata],axis=1,join='inner')
    ds = SWCdata
    # ds1 = pd.concat([monthtimedata, daytimedata, Pdata, Tsdata], axis=1, join='inner')
    # ds = Tsdata
...
def main(datadir,dataname,lead_time,
    ...
    #                   "l1_ratio": [0, 0.1, 0.3, 0.5, 0.7, 0.9, 1, 2]}
    # parametersGrid = {"max_iter": [1, 100, 200],
    #                   "alpha": [ 0.1, 1, 10, 100],
    #                   "l1_ratio": [0,  0.5, 1]}
