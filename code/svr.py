...
def generatedatafluxnet(datadir, dataname):
    ...
    ds1 = pd.concat(
        [yeartimedata, monthtimedata, daytimedata, Tsdata, Lr, Sr, AT, AP, WS, Pdata, P_oneweek_data, SWCdata], axis=1,
        join='inner')
    # ds1 = pd.concat([yeartimedata,monthtimedata,daytimedata,Lr,AP,WS,Pdata,SWCdata],axis=1,join='inner')

    # ds1 = pd.concat([monthtimedata, Tsdata, Pdata, SWCdata], axis=1, join='inner')
    ds = SWCdata
...
def main(datadir, dataname, lead_time,
    ...
    # 初始化SVR
    svr.fit(svr_input, data_train_y)
    ...
    pred = svr.predict(svr_input_test)
...
if __name__ == '__main__':
    ...
    p.add_argument('--dataname', type=str, default='FLX_NL-Loo', help='Location to data')
    ...
    p.add_argument('--modelname', type=str, default='svr-5days-daily-prediciton', help='name for prediction model')
    args = p.parse_args()
